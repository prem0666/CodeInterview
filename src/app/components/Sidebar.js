          ...
          {navItems.map(({ icon: Icon, label, href }) => {
            ...
            return (
              <Link key={label} href={href} onClick={onClose}
                ...
                className={`
                  relative flex items-center gap-3 rounded-lg transition-all duration-150 group overflow-hidden
                  ${collapsed ? "w-10 h-10 justify-center" : "px-3 py-2.5"}
                  ${active
                    ? "bg-indigo-500/15 text-indigo-200 border border-indigo-500/25 shadow-[0_0_18px_rgba(99,102,241,0.12)]"
                    : "text-slate-500 hover:text-slate-100 hover:bg-white/5 border border-transparent"}
                `}>
                {/* Active left bar */}
                {active && (
                  <span className={`absolute left-0 top-1/2 -translate-y-1/2 w-0.5 rounded-r-full bg-indigo-400 shadow-[0_0_10px_rgba(99,102,241,0.75)] ${collapsed ? "h-5" : "h-7"}`} />
                )}
                <Icon className={`w-4.5 h-4.5 shrink-0 ${active ? "drop-shadow-[0_0_8px_rgba(99,102,241,0.9)]" : ""}`} />
